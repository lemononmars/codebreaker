...
<svelte:head>
	<title>Code Breaker | Login</title>
</svelte:head>
	...
	<div class="card w-96 bg-neutral text-neutral-content shadow-xl">
		<div class="card-body items-center text-center">
			...
			<div class="w-full text-right mt-2">
				<a href="/reset-password" class="text-sm link link-primary opacity-70 hover:opacity-100"
					>Forgot password?</a
				>
			</div>

			{#if message}
				<div class="alert alert-error shadow-lg mt-4 text-sm">
					<div>
						<span>{message}</span>
					</div>
				</div>
			...
			<div class="card-actions justify-end mt-6 w-full">
				<button class="btn btn-primary w-full {loading ? 'loading' : ''}" on:click={handleLogin}>
					Login
				</button>
			</div>
