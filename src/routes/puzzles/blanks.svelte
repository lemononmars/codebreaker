					...
					{#if answerStatus === 'correct'}
						...
						>
							...
							ถูกต้อง! {#if gameMode === 'timeattack'}<span
									class="text-warning font-black ml-1 animate-bounce">+3s ⚡</span
								>{/if}
						</div>
					{:else if answerStatus === 'incorrect'}
						<div class="flex flex-col items-center gap-1 animate-in fade-in slide-in-from-bottom-3 duration-200">
							<div class="flex items-center gap-2.5 text-error font-black text-base sm:text-xl">
								<XCircleIcon size="22" />
								{timeLeft <= 0 ? 'หมดเวลา!' : 'คำตอบไม่ถูกต้อง!'}
							</div>
							<span class="text-xs text-neutral-content/75 font-semibold">
								คำตอบที่ถูก: <b class="text-white text-sm">{revealedCorrectAnswer}</b>
							</span>
						</div>
					{/if}
				</div>
