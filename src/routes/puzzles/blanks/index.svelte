...
<div id="blanks-game-container" class="max-w-4xl mx-auto px-2 sm:px-4 py-2 sm:py-4 flex flex-col gap-3 select-none">
	<div class="bg-neutral text-neutral-content border border-base-300 rounded-3xl p-3 sm:p-6 shadow-2xl flex flex-col gap-3 relative overflow-hidden text-center">
		...
		{#if currentMode === 'selection'}
			<div class="flex items-center justify-between gap-2 border-b border-base-300/80 pb-2 text-left">
				<div class="flex items-center gap-2 overflow-hidden">
					...
					<h1 class="text-base sm:text-xl font-extrabold text-neutral-content tracking-tight truncate">
						ซ่อนอักษร <span class="text-xs text-neutral-content/70 font-medium hidden sm:inline">(เติมคำในช่องว่าง)</span>
					</h1>
				</div>
				...
				<div class="flex items-center gap-1 shrink-0">
					...
					<a href="/puzzles/blanks/leaderboard" class="btn btn-ghost btn-xs sm:btn-sm gap-1 text-base-content hover:bg-base-200">
						<AwardIcon size="16" />
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full text-left my-2">
				...
				<!-- Difficulty Select -->
				<div class="flex flex-col gap-2">
					<label for="difficulty" class="text-sm font-extrabold text-neutral-content flex items-center gap-1.5 px-1">
						⚙️ ระดับความยาก
					</label>
					<select
						id="difficulty"
			...
			{#if currentHighScore > 0}
				<div class="text-xs sm:text-sm font-bold text-cyan-400 bg-slate-950/60 p-3 rounded-2xl border border-slate-800">
