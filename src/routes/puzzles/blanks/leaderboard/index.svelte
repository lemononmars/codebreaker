<script lang="ts">
	...
	let selectedGlobalMode = 'normal';
	let selectedGlobalDifficulty = 'normal';
	...
	async function fetchGlobal() {
		...
		globalError = false;
		try {
			const res = await fetch('/api/leaderboard/blanks');
	...
	$: filteredGlobalEntries = globalEntries
		.filter(entry => entry.puzzle_id === getPuzzleId(selectedGlobalMode, selectedGlobalDifficulty))
...
<svelte:head>
	...
	<meta name="description" content="ตารางคะแนนสูงสุด เกมซ่อนอักษร แยกตามโหมดและระดับความยาก" />
</svelte:head>
...
<div class="flex flex-col gap-8 w-full max-w-3xl mx-auto px-4 pb-16 pt-4">
		...
		<div class="flex flex-col gap-1">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-xs sm:text-sm opacity-60 font-semibold">เกมซ่อนอักษร</p>
				</div>
			</div>
		</div>
	</div>
	...
	{:else}
		<section class="flex flex-col gap-4">
			...
			<!-- Filter Controls -->
			<div class="grid grid-cols-2 gap-3 bg-base-300/10 p-3 rounded-2xl border border-base-300/30">
				<div class="flex flex-col gap-1.5 text-left">
					<label for="globalMode" class="text-[10px] font-black uppercase opacity-55 tracking-wider px-1">รูปแบบการเล่น</label>
					<select
						id="globalMode"
						bind:value={selectedGlobalMode}
						class="select select-bordered select-sm w-full bg-neutral text-neutral-content border-base-300 font-extrabold focus:border-primary rounded-xl cursor-pointer"
					>
				...
				<div class="flex flex-col gap-1.5 text-left">
					<label for="globalDiff" class="text-[10px] font-black uppercase opacity-55 tracking-wider px-1">ระดับความยาก</label>
					<select
						id="globalDiff"
						bind:value={selectedGlobalDifficulty}
						class="select select-bordered select-sm w-full bg-neutral text-neutral-content border-base-300 font-extrabold focus:border-primary rounded-xl cursor-pointer"
					>
						<option value="normal">ทั่วไป (1 ตัวอักษร)</option>
						<option value="difficult">ท้าทาย (2 ตัวอักษร)</option>
					</select>
				</div>
			</div>
			...
			{:else if filteredGlobalEntries.length === 0}
				<div class="flex flex-col items-center gap-4 py-12 opacity-50">
					<span class="text-5xl">🎮</span>
					<p class="font-bold text-center">ยังไม่มีคะแนนในโหมดนี้<br/>เป็นคนแรกที่ส่งคะแนน!</p>
					<a href="/puzzles/blanks" class="btn btn-primary btn-sm">เล่นเลย</a>
				</div>
			{:else}
				<div class="card bg-neutral shadow-xl border border-base-300 overflow-hidden">
					<table class="table w-full">
						...
						<tbody>
							{#each filteredGlobalEntries as entry, i}
