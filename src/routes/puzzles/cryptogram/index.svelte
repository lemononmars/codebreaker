<script lang="ts">
	...
	export let puzzles: ICryptogram[] = [];
</script>
...
<div class="container mx-auto px-4 py-6 flex flex-col gap-6 max-w-5xl">
	<div class="text-center mb-2">
		<h1 class="text-3xl lg:text-4xl font-extrabold tracking-tight text-white">Cryptogram</h1>
		<p class="text-xs sm:text-sm text-slate-400 mt-1">ถอดรหัสข้อความลับซ่อนคำ</p>
	</div>
					...
					{#each puzzles as p}
						<tr class="hover:bg-slate-800/40 transition-colors">
							<td class="py-3.5 px-4 font-bold text-emerald-400 hover:text-emerald-300">
								<a href="/puzzles/cryptogram/{p.id}">
									{p.title}
								</a>
							</td>
