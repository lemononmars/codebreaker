<script lang="ts">
	...
	export let puzzles: IMissingVowels[] = [];
</script>

<svelte:head>
	<title>Code Breaker | Missing Vowels</title>
</svelte:head>
...
<div class="container mx-auto px-4 py-6 flex flex-col gap-6 max-w-5xl">
	<div class="flex flex-col sm:flex-row justify-between items-center gap-4 border-b border-slate-800 pb-4">
		<div>
			<h1 class="text-3xl lg:text-4xl font-extrabold tracking-tight text-white">Missing Vowels</h1>
			<p class="text-xs sm:text-sm text-slate-400 mt-1">ทายคำจากพยัญชนะที่ไม่มีสระ</p>
		</div>
		<a href="/tools/missingvowels" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs shadow-md transition-all">
					...
					{#each puzzles as p}
						<tr class="hover:bg-slate-800/40 transition-colors">
							<td class="py-3.5 px-4 font-bold text-emerald-400 hover:text-emerald-300">
								<a href="/puzzles/missingvowels/{p.id}">
									{p.title}
								</a>
							</td>
