<script lang="ts">
	...
	$: sortedData = [...leaderboardData].sort((a, b) => a.created_at.localeCompare(b.created_at));
	...
	<tbody>
		{#each sortedData as d, idx}
			<tr class={idx <= 5 ? 'bg-slate-50' : ''}>
				...
				{#if d.name.length > 15}
					<th
						><a href="/user/{d.name}" class="link link-hover"
							><div class="tooltip" data-tip={d.name}>{d.name.slice(0, 20) + '...'}</div></a
						></th
					>
				{:else}
					<th><a href="/user/{d.name}" class="link link-hover">{d.name}</a></th>
				{/if}
